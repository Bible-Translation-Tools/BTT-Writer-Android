...
Then fork this repository and clone your fork. After the repository has been cloned to your computer create a new Genymotion emulator. These emulators are faster and have better features that the stock emulators that come with the Android SDK. As always a physical device will provide the best experience if you have one.
...
## Building
This application relies on several native libraries. Therefore when building for a device or emulator you must choose the correct build variant for that platform. In Android Studio you can change the build variants by clicking on the `Build Variants` tab in the lower left corner of the IDE window. This will display an embedded window in which you can choose the correct build variant for the `app` module.
